...
namespace KHJ.Passive
{
    ...
    public class SeriousPunchPassiveSO : PassiveSO, IActivePassive, ICooldownPassive, IEffectPoolPassive
    {
        public EventReference seriousPunchSound;
        ...
        private void HandleHitEvent(HitInfo info)
        {
            ...
            FindEnemiesInRange(target.transform, _radius).ForEach(enemy =>
            {
                enemy.HealthCompo.ApplyOnlyDamage(newValue);
                RuntimeManager.PlayOneShot(seriousPunchSound, enemy.transform.position);
            });
